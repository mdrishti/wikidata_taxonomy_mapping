...
## query below fails because ofmore than 1 mio hits with the label service. Couldn't resolve it for now.
queryFail <- '{PREFIX wdt: <http://www.wikidata.org/prop/direct/> 
		SELECT ?WdID ?eol ?gbif ?ncbi ?ott ?itis ?irmng ?col ?nbn ?worms ?bold ?plazi ?apni ?WdName WHERE{ 
		?WdID wdt:P31 wd:Q16521;
			wdt:P225 WdName .
			        OPTIONAL { ?WdID wdt:P9187 ?ott . }
			        ...
			        OPTIONAL { ?ott wdt:P3151 ?inat . } #to save pid
			SERVICE wikibase:label { bd:serviceParam wikibase:language "en". }
		        }'
...
df.otol <- read.csv("ott3.6/taxonomy_135_DT.tsv",header=TRUE,row.names=NULL, sep="\t")
...
df.otol.wd <- full_join(df.Wdfull,df.otolY, by=c("ott"="ott_id"), relationship="many-to-many",suffix=c(".wd",".otol")) #all wd ids mapping to 11 dbs, but also ott not matching to wd
write.csv(df.otol.wd, "all_wd_eol_ncbi_gbif_andOthers_mapping.txt", row.names=FALSE)

#simple metrics to check number of unique ott-ids and wd-ids mapped
ottU <- length(unique(df.otol.wd[which(is.na(df.otol.wd$WdID)==FALSE & is.na(df.otol.wd$ott)==FALSE),c("ott")])) #unique ott that have wd
WdU <- length(unique(df.otol.wd[which(is.na(df.otol.wd$WdID)==FALSE & is.na(df.otol.wd$ott)==FALSE),c("WdID")])) #unique wd that have ott
ottX <- length(unique(df.otol.wd[which(is.na(df.otol.wd$WdID)==FALSE & is.na(df.otol.wd$ott)==FALSE),c("ott")])) #unique ott that don't have wd
